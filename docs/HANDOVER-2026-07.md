...
| T12 | 実写AR合成の整列品質が未達(全然重ならない) | ユーザー向け非公開化(`/mirror/<code>?dev=1`でのみトグル表示)。機能・コードは温存。改善方向: カメラFOV実測合わせ、z=0平面固定をやめ肩幅から奥行き推定、四肢ごとの2Dフィッティング、Image Segmenterでの人物マスク合成 |
