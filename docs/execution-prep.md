...
- ロードマップは `docs/roadmap.md` を単独で更新する。
- 日次進捗は `docs/progress-log.md` に追記する。
- 大きな仕様変更時は本ファイルに決定理由を追記する。

## 7. 追加決定（2026-02-28）

- 体装着ロジックは右腕専用ではなく、セグメント共通ロジック（全身）へ汎化する。
- 画像生成は三面図中心から、部位別パーツ生成（module単位）を主系にする。
