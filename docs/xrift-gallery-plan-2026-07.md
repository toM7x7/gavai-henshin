...
| 2 | ✅実装済(2026-07-20): **格納庫ホール**(アイアンマン格納庫の意匠) — ①壁面アルコーブ6基(背光フレーム+台座、**訪問ごとにシャッフル**)②**中央召喚台**: TextInputコンソールに呼出符5文字→任意スーツ召喚、**蒸着ボタン(Interactable)でVRMAヘンシンモーション再生**。召喚・蒸着はuseInstanceStateで**来訪者全員に同期**③負荷設計: 展示上限6+**逐次ロードキュー**(1体ずつ)+入替時のGPUメモリ返却(deepDispose)④【試験中】タイトル+ユーザーライク説明+実レンダ合成サムネ(make_thumbnail.py)。※ローカルペインはrAF停止でR3F初回コミット不可のため実走は本番XRiftで確認 | Phase 1完了済 |
...
- **来場のたびに重くなる?** → ワールド本体は約5.5MBで固定(実行時fetch方式なので
  収蔵が増えてもワールドの再アップロードは無く、サイズは増えない)。重いのは
  **来場ごとのVRMダウンロード**(展示数×10MB級)で、収蔵総数には比例しない
  (展示上限6+召喚1)。ブラウザキャッシュが再訪問を軽くする。将来: 展示専用の
  軽量LOD(forgeでlod2書き出し)で1桁圧縮可
- **ワールド詳細画面に接続先ドメインが表示される件** → セキュリティ上の実害なし:
  ①このURLはWebアプリのクライアントJSにも既に含まれる公開情報(公開バケット)
  ②表示は「訪問者が緩和内容に同意して入室する」XRiftの透明性設計で、隠す方が
  設計思想に反する ③本当の防御はドメインの表示有無ではなくRLS/書込鍵の分離(済/宿題)。
  見え方が気になるなら**独自ドメインのプロキシ**(例: assets.<自ドメイン> →
  Supabaseへrewrite)を挟めば表示もユーザーライクになり、CDNキャッシュと
  差し替え自由度も得られる — 公開ベータ前の推奨施策
- **スーツのフォルム収斂(ギャラリー外の課題)** → 三角形数(30〜54万)とパレットは
  バラつく=完全同一ではなく**シルエットの収斂**。有力仮説: route B(Gemini)の
  設計図が形状DNA(シルエット族/断面/クレスト)を埋めず、修復層が毎回同じ既定値で
  補完している。確認: Cloud Runログで直近ジョブのblueprint内容(profiles/families/
  cross_section)とroute表示。対策: **route B修復層に意図文シードの形状変奏を注入**
  (route Aの変奏レバー共通化)→ 要エンジン修正+forge3段階再デプロイ
